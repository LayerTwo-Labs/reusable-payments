...
pub fn derive_bip47_lookup_entries<C: Signing + Verification>(
...
/// Parse a block (with resolved prevouts) into the scanner's per-transaction
/// projection. Prevout resolution is the caller's responsibility.
pub fn block_to_scanned_txs(
    block: &bitcoin::Block,
    prevouts: &HashMap<OutPoint, bitcoin::TxOut>,
) -> Vec<ScannedTx> {
    block
        .txdata
        .iter()
...
mod tests {
    ...
    fn bip352_official_receiving_vectors() {
        const VECTORS: &str = crate::silent_payments_test_vectors::VECTORS;
