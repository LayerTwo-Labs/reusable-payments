...
pub fn sp_path(coin: u32, account: u32, branch: u32) -> Result<DerivationPath, CryptoError> {
    let components = [
...
/// BIP352 input eligibility: P2TR, P2WPKH, or P2PKH script types.
        ...
        for (case_idx, case) in cases.iter().enumerate() {
            ...
            for (sub_idx, send) in sending_arr.iter().enumerate() {
                ...
                for v in vin {
                    ...
                    if let Some(pk) = crate::scan::extract_input_pubkey(
                        &witness,
